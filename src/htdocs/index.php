...
if($submit){
    ...
    if(count($error) > 0){
        ...
        $data['error'] = $error;
        escape($data['error']);
        //headerでリダイレクトさせないなら、POSTの値がviewに受け渡されるので、sessionを使う必要はない。
        ...
        //reloadするとpostされたデータはリセットされるので必要ない。
...
//viewをさいごに読み込む事で、$dataをvalidationした状態でviewにうけわたせる。
//$view = view('index_view.php',$data);
//echo $view;

include_once 'system/view/index_view.php';
